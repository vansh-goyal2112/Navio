...
// Import authentication context to access current logged-in user
 ...
 * --------------------------------------------------
 * Purpose:
 * - Displays authenticated user's profile details
 * - Shows account-related information (name, email, UID)
 * - Provides profile summary and system status
...
export default function ProfilePage() {
   ...
   * - Prevent access to profile data
   */
  if (user === null) {
    return (
  ...
  /**
   * Main Profile UI (Authenticated Users Only)
   * --------------------------------------------------
   * Sections:
   * 1. Profile header (title + optional profile image)
   * 2. Account information (name, email, UID)
   * 3. Profile summary and system status
   */
  return (
    <main className="space-y-8">

      {/* PROFILE HEADER SECTION */}
      {/* Displays title, description, and optional profile image */}
      <section className="rounded-[28px] border border-[#E2E8F0] bg-white p-8 shadow-lg dark:border-[#334155] dark:bg-[#1E293B]">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* Right side: profile image (only if available) */}
          {user.photoURL && (
            <div className="flex justify-start sm:justify-end">
              <img
                src={user.photoURL} // User profile image URL
                alt="User profile"
      ...
      {/* MAIN CONTENT GRID */}
      {/* Left: account details | Right: summary + status */}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-5 dark:border-[#334155] dark:bg-[#0F172A]">
              ...
              <p className="mt-2 text-lg font-semibold text-[#0F172A] dark:text-white">
                {/* Fallback if display name not available */}
            ...
            {/* USER ID FIELD (conditionally rendered) */}
            {user.uid && (
              <div className="rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-5 dark:border-[#334155] dark:bg-[#0F172A]">
